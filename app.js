...
function startTest(id) {
  if (!currentStudent) {
    const saved = localStorage.getItem('asamblari-currentStudent');
    if (saved) {
      currentStudent = JSON.parse(saved);
      currentTest = id; currentQ = 0; score = 0; renderQuestion();
    } else {
...
function showResult() {
  ...
  document.getElementById('mainContent').innerHTML = `<div class="container"><div style="max-width:600px;margin:0 auto;text-align:center;padding:3rem 1rem"><div class="result-score">${pct}%</div><p class="result-message">${pct >= 70 ? '🎉 Felicitări! Ai promovat!' : '📚 Mai exersează și încearcă din nou!'}</p><div class="result-details"><div class="result-stat"><div class="result-stat-number correct">${score}</div><div class="result-stat-label">Corecte</div></div><div class="result-stat"><div class="result-stat-number incorrect">${total - score}</div><div class="result-stat-label">Greșite</div></div></div><div class="info-box" style="text-align:left;margin:2rem 0"><h4>📊 Statistici Globale</h4><p>Teste completate: <strong>${stats.completed}/9</strong> | Media: <strong>${stats.avgPct}%</strong> | Total încercări: <strong>${stats.attempts}</strong></p></div><div class="mt-4"><button class="btn btn-primary" onclick="startTest('${currentTest}')">🔄 Reia Testul</button> <button class="btn btn-secondary" onclick="showSection('home')">🏠 Acasă</button></div></div></div>`;
  sendResultEmail(ch.title, pct, score, total - score);
}
...
function sendResultEmail(testName, percentage, correct, wrong) {
  ...
  fetch(url, {
    ...
    .catch(err => {
      ...
      alert('Eroare la trimiterea rezultatului, asigurați-vă că sunteți conectat la internet.');
    });
}
...
function showExamResult() {
  ...
  sendResultEmail('Examen Final', pct, score, total - score);
}
  ...
  main.innerHTML = `
    <div class="container">
      ...
      <div style="text-align:center; margin-bottom: 2rem;">
        <button class="btn btn-primary" onclick="sendResultEmail('Scor Medalii', Math.round((earned.length / allMedals.length) * 100), earned.length, allMedals.length - earned.length)">📧 Trimite Progres Profesorului</button>
      </div>
